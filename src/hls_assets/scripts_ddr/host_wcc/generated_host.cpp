...
void AlgorithmHost::prepare_data(const PartitionContainer &container,
    ...
    h_labels.assign(m_num_vertices, 0);
    if (start_node < m_num_vertices) {
        h_labels[start_node] = 1;
    }
    ...
    for (int i = 0; i < container.num_sparse_partitions; ++i) {
        ...
        // Pack node distances ONCE for the big partition (shared)
        {
            ...
            for (int j = 0; j < big_partition.num_vertices; ++j) {
                ...
                ap_fixed_pod_t dist_val = h_labels[global_id];
    ...
    for (int i = 0; i < container.num_dense_partitions; ++i) {
        ...
        for (int pip = 0; pip < LITTLE_KERNEL_NUM; ++pip) {
            ...
            printf("LITTLE SP No.%d, pipe No.%d, edge words %d\n", i, pip,
                   (int)num_edge_words);
            printf("Allocating mem size: %d bytes\n",
                   (int)(num_edge_words * bytes_per_word));
...
bool AlgorithmHost::check_convergence_and_update(
    ...
    for (auto const &[global_id, new_dist] : min_distances) {
        if (global_id < m_num_vertices && new_dist < h_labels[global_id]) {
            h_labels[global_id] = new_dist;
            changed = true;
        }
    }
...
const std::vector<int> &AlgorithmHost::get_results() const {
    ...
    final_distances.reserve(h_labels.size());

    for (const auto &dist : h_labels) {
...
bool AlgorithmHost::check_convergence_and_update(
    ...
    std::map<int, ap_fixed_pod_t> merged_labels;
    ...
    auto consume_partition_dsts = [&](const PartitionDescriptor &partition) {
        for (int local_id = 0; local_id < partition.num_dsts; ++local_id) {
            ...
            ap_fixed_pod_t out_prop =
                apply_kernel_host_outputs[word_idx].range(bit_offset + 31,
                                                          bit_offset);
            ...
            if (it != partition.vtx_map_rev.end()) {
                ...
                if (global_id < m_num_vertices) {
                    auto found = merged_labels.find(global_id);
                    if (found == merged_labels.end() ||
                        out_prop > found->second) {
                        merged_labels[global_id] = out_prop;
                    }
                }
            }
    ...
    for (const auto &[global_id, new_label] : merged_labels) {
        if (global_id < m_num_vertices) {
            if (h_labels[global_id] != new_label) {
                h_labels[global_id] = new_label;
                changed = true;
            }
        }
    }
    ...
    if (changed) {
        std::cout << "[INFO] Labels updated. Preparing for next iteration."
                  << std::endl;
    } else {
        std::cout << "[INFO] No label updates. Algorithm has converged."
                  << std::endl;
    }
...
const std::vector<unsigned int> &AlgorithmHost::get_results() const {
    static std::vector<unsigned int> final_labels;
    final_labels.clear();
    final_labels.reserve(h_labels.size());
    for (const auto &label : h_labels) {
        final_labels.push_back((uint32_t)label);
    }
    return final_labels;
}
