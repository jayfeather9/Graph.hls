...
void AlgorithmHost::prepare_data(const PartitionContainer &container,
    ...
    avg_outdegree = (m_num_vertices > 0)
                        ? (uint32_t)(total_outdegree / (uint64_t)m_num_vertices)
                        : 0;

    // AR: initialize contributions to 0
    h_contrib.assign(m_num_vertices, 0);
    ...
    for (int i = 0; i < container.num_sparse_partitions; ++i) {
        ...
        // Pack node distances ONCE for the big partition (shared)
        {
            ...
            for (int j = 0; j < big_partition.num_vertices; ++j) {
                ...
                int32_t dist_val = h_contrib[global_id];
            ...
            for (int j = 0; j < big_partition.num_dsts; ++j) {
                ...
                int global_id = big_partition.vtx_map_rev.at(j);
                uint32_t dist_val = out_degree[global_id];
                const char *data_ptr =
                    reinterpret_cast<const char *>(&dist_val);
                temp_byte_buffer.insert(temp_byte_buffer.end(), data_ptr,
                                        data_ptr + bytes_per_dist);
...
void AlgorithmHost::execute_kernel_iteration(
    ...
    // big_merger)
    {
        ...
        OCL_CHECK(err, err = apply_kernel.setArg(arg_idx++, avg_outdegree));
...
bool AlgorithmHost::check_convergence_and_update(
    ...
    constexpr int kScaleDegree = 16;
    constexpr int kScaleDamping = 7;
    constexpr int32_t kDampFixPoint = 108; // 0.85 * 128
    constexpr int32_t kBase =
    ...
    auto consume_partition_dsts = [&](const PartitionDescriptor &partition) {
        for (int local_id = 0; local_id < partition.num_dsts; ++local_id) {
            ...
            uint32_t out_prop_u = (uint32_t)apply_kernel_host_outputs[word_idx]
            ...
            if (it != partition.vtx_map_rev.end()) {
                ...
                if (global_id < m_num_vertices) {
                    merged_contrib[global_id] = (int32_t)out_prop_u;
                }
            }
    ...
    for (const auto &[global_id, new_val] : merged_contrib) {
        if (global_id < m_num_vertices) {
            int32_t old_val = h_contrib[global_id];
            int32_t delta = new_val - old_val;
            uint32_t abs_delta =
                (delta < 0) ? (uint32_t)(-(int64_t)delta) : (uint32_t)delta;
            if (abs_delta > max_abs_delta) {
                max_abs_delta = abs_delta;
            }
            h_contrib[global_id] = new_val;
        }
    }

    // Vertices with indegree==0 are never destinations (not in any partition's
    // dst list), so the FPGA won't write them. Update them on host with sum_in=0.
    for (int v = 0; v < m_num_vertices; ++v) {
        if (!has_incoming[v]) {
            ...
            if (denom != 0) {
                ...
                int32_t new_score =
                    (int32_t)((int64_t)kDampFixPoint * 0 + (int64_t)kBase);
                new_contrib = (int32_t)((int64_t)new_score * (int64_t)tmp);
            }
            int32_t old_val = h_contrib[v];
            int32_t delta = new_contrib - old_val;
            uint32_t abs_delta =
                (delta < 0) ? (uint32_t)(-(int64_t)delta) : (uint32_t)delta;
            if (abs_delta > max_abs_delta) {
                max_abs_delta = abs_delta;
            }
            h_contrib[v] = new_contrib;
        }
    }

    bool converged = (max_abs_delta <= kMaxAbsDeltaEps);
    if (!converged) {
...
const std::vector<unsigned int> &AlgorithmHost::get_results() const {
    static std::vector<unsigned int> final_props;
    final_props.clear();
    final_props.reserve(h_contrib.size());
    for (int32_t v : h_contrib) {
        final_props.push_back((uint32_t)v);
    }
    return final_props;
}
