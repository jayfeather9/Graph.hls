        ...
        for (int pip = 0; pip < BIG_KERNEL_NUM; ++pip) {
            ...
            if (pipeline_edges.num_edges == 0) {
                ...
                for (int dummy_idx = 0; dummy_idx < 8; ++dummy_idx) {
                    ...
                    uint64_t packed_edge =
                        ((uint64_t)dummy_src_id << 32) |
                        ((uint64_t)0 << 22) |
                        (uint64_t)(dummy_dst_id & 0x3FFFFF);

                    for (int b = 0; b < bytes_per_edge; ++b) {
                        edge_bytes[b] = (packed_edge >> (8 * b)) & 0xFF;
                    }
                ...
                for (int v = 0; v < big_partition.num_vertices; ++v) {
                    ...
                    for (int edge_idx = pipeline_edges.offsets[v];
                        ...
                        int32_t weight_raw = pipeline_edges.weights[edge_idx];
                        uint32_t weight_clamped = 0;
                        if (weight_raw > 0) {
                            weight_clamped = (weight_raw > 1023)
                                                 ? 1023
                                                 : (uint32_t)weight_raw;
                        }
                        uint64_t packed_edge =
                            ((uint64_t)src_id << 32) |
                            ((uint64_t)(weight_clamped & 0x3FF) << 22) |
                            (uint64_t)(dest_id & 0x3FFFFF);
