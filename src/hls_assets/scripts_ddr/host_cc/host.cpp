...
int main(int argc, char **argv) {
    ...
    int iter_count = 0;
    std::vector<unsigned int> fpga_masks = run_fpga_kernel(
    ...
    std::vector<unsigned int> host_masks = verify_on_host(graph, start_node);

    // 4. 比较结果 (不变)
    std::cout << "\n--- Step 4: Comparing Results ---" << std::endl;
    int error_count = 0;
    for (int i = 0; i < graph.num_vertices; ++i) {
        if (fpga_masks[i] != host_masks[i]) {
            if (error_count < 10) {
                std::cout << "Mismatch at vertex " << i << ": "
                          << "FPGA_Result = " << fpga_masks[i] << ", "
                          << "Host_Result = " << host_masks[i] << std::endl;
            }
            error_count++;
        }
    }
