...
void AlgorithmHost::prepare_data(const PartitionContainer &container,
    ...
    // ReGraph-style PR: store contrib = rank/outdeg in fixed-point (scale=2^30)
    const uint64_t SCALE = (1ull << 30);
    uint32_t inv_n = (m_num_vertices > 0) ? (uint32_t)(SCALE / (uint64_t)m_num_vertices) : 0;
    pr_base_arg = (m_num_vertices > 0)
                      ? (uint32_t)((SCALE * 15ull) / (100ull * (uint64_t)m_num_vertices))
                      : 0; // (1-d)/N with d=0.85

    h_contrib.assign(m_num_vertices, 0);
    for (int u = 0; u < m_num_vertices; ++u) {
        uint32_t od = out_degree[u];
        h_contrib[u] = (od != 0) ? (int32_t)(inv_n / od) : 0;
    }
    ...
    for (int v = 0; v < m_num_vertices; ++v) {
        if (!has_incoming[v]) {
            ...
            if (od != 0) {
                ...
                new_contrib = (int32_t)(((int64_t)(int32_t)pr_base_arg * (int64_t)tmp) >> 16);
            }
