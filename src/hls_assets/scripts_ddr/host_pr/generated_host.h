...
class AlgorithmHost {
  public:
    ...
    // ---
    void prepare_data(const PartitionContainer &container, const GraphCSR &graph,
                      int start_node);
  ...
  private:
    ...
    // PR state (ReGraph-style): store per-vertex contribution = rank/outdegree
    std::vector<int32_t> h_contrib;
    std::vector<uint32_t> out_degree;
    std::vector<uint8_t> has_incoming;
    uint32_t pr_base_arg = 0; // fixed-point base term (same scale as h_contrib)
