...
void apply_func(bus_word_t *node_props, uint32_t arg_reg, hls::stream<in_write_burst_w_dst_pkt_t> &write_burst_stream, hls::stream<write_burst_w_dst_pkt_t> &kernel_out_stream) {
    LOOP_WHILE_44: while (true) {
        ...
        if (in_pkt.end_flag) {
            ...
            LOOP_FOR_43: for (int32_t i = 0; (i < DIST_PER_WORD); ++i) {
                ...
                ap_uint<32> outDeg = (ap_uint<32>)ori_props.range(31 + (i << 5), (i << 5));
                const ap_int<32> kDampFixPoint = 108;
                ap_int<32> new_score = (ap_int<32>)arg_reg + (ap_int<32>)(((ap_int<64>)kDampFixPoint * (ap_int<64>)sum_in) >> 7);
                ap_int<32> new_contrib = 0;
                if (outDeg != 0) { ap_uint<32> tmp = ((ap_uint<32>)1 << 16) / outDeg; new_contrib = (ap_int<32>)(((ap_int<64>)new_score * (ap_int<64>)tmp) >> 16); }
...
extern "C" void apply_kernel(bus_word_t *node_props, uint32_t little_kernel_length, uint32_t big_kernel_length, uint32_t little_kernel_st_offset, uint32_t big_kernel_st_offset, uint32_t arg_reg, hls::stream<write_burst_pkt_t> &little_kernel_out_stream, hls::stream<write_burst_pkt_t> &big_kernel_out_stream, hls::stream<write_burst_w_dst_pkt_t> &kernel_out_stream) {
