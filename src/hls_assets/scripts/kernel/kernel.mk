...
ifeq ($(TARGET),hw_emu)
CLFLAGS += -DGRAPHYFLOW_HW_EMU_LIMIT_MAX_DST
endif
...
LDFLAGS_VPP += -O3
LDFLAGS_VPP += --vivado.synth.jobs $(VIVADO_SYNTH_JOBS)
...
$(XCLBIN_FILE): $(KERNEL_XOS) system.cfg
	@echo "==> 正在将所有内核链接到 xclbin 文件: $@"
	$(VPP) -l -t $(TARGET) --platform $(DEVICE) $(LDFLAGS_VPP) -o $@ $(KERNEL_XOS)
...
$(XCLBIN_DIR)/%.$(TARGET).xo: scripts/kernel/%.cpp
	@echo "==> 正在编译内核: $<"
	@mkdir -p $(XCLBIN_DIR)
	$(VPP) -c -t $(TARGET) --platform $(DEVICE) --jobs $(VPP_JOBS) $(CLFLAGS) --kernel $* -o $@ $<
